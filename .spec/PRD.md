...
Nureaknite is a personal brand website for a single knitting and crochet designer, educator, and content creator. The platform serves as a professional digital studio to showcase work, build brand trust, share knowledge, and grow an audience.
...
The system encompasses a public-facing website for content consumption and product showcase, user accounts for managing downloads and wishlists, and an admin interface for content management.

| Role | Description |
|:------------------|:-------------------------------------------------------------------------|
| **Guest** | Anonymous visitor. Can browse public content and products. |
| **Registered User** | Authenticated user. Can download patterns, save wishlist items, and manage profile. |
| **Admin (Creator)** | The sole owner and manager of the website with full content control. |
...
| Manage All Site Content (CMS) | ❌ | ❌ | ✅ |
| View Coaching Requests | ❌ | ❌ | ✅ |

## Functional Requirements

### User-Facing Requirements
*   **FR-01 (Authentication):** Users can register, log in, and log out using an email and password. Authentication is managed via NextAuth.js.
...
### Admin-Facing Requirements (via Headless CMS - Strapi)
*   **FR-09 (Content Management):** The Admin can perform full CRUD (Create, Read, Update, Delete) operations on all content models: Patterns, Blog Posts, Portfolio Items, and Shop Products.
...
| | Infrastructure | Vercel/Render infrastructure must support auto-scaling or simple vertical scaling. |
...
| **Backend CMS** | Strapi (Node.js) | Flexible headless CMS enables the creator to manage diverse content types from a single interface. |
| **Database** | Supabase (PostgreSQL) | Integrated solution providing a managed Postgres DB, simplifying backend infrastructure needs. |
| **Authentication**| NextAuth.js | Robust, secure authentication library tightly integrated with the Next.js ecosystem. |
| **File Storage** | Supabase Storage | Secure, integrated storage for PDF patterns and images, managed alongside the database and auth. |
| **Email** | Resend | Transactional emails (coaching notifications, pattern download links). |
| **Hosting** | Vercel (FE) & Render (BE) | Best-in-class, developer-centric hosting platforms for Next.js and Node.js with seamless CI/CD. |
...
*   **Constraint:** The website will be managed by a single Admin (the creator). The admin interface must be intuitive for a non-technical user.
...
The initial release of Nureaknite will **not** include:
